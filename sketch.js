...
function drawChart() {
  ...
  d3.csv("ABI_Einsatzbetriebe_und_Einsatzplaetze_nach_TB_2025.csv").then(raw => {
    ...
    const rowHeight = isMobile ? 70 : 88;
    const legendHeight = 122; // ⭐ légende désormais au-dessus du graphique, sur 3 lignes
    const margin = {
      top: legendHeight,
      right: isMobile ? 60 : 90,
      bottom: 20,
      left: isMobile ? 170 : 340
    };
    ...
    legendItems.forEach((item, i) => {
      const row = legend.append("g").attr("transform", `translate(0, ${i * 52})`);
      row.append("rect")
        .attr("width", 10)
        .attr("height", 10)
        .attr("y", 4)
      ...
      const t = row.append("text")
        .style("font-family", "Arial")
        .style("font-size", isMobile ? "10.5px" : "12.5px")
        .style("fill", "#111");

      t.append("tspan").attr("x", 18).attr("dy", "0.8em").text(parts[0]);
      t.append("tspan").attr("x", 18).attr("dy", "1.15em").text(parts[1]);
      t.append("tspan").attr("x", 18).attr("dy", "1.15em").text(parts[2]);
    });
    ...
    rowLabels.each(function (d) {
      ...
      group.append("tspan").attr("x", -10).attr("dy", "-1.1em").text(parts[0]);
