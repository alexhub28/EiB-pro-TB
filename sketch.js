...
function drawChart() {
  ...
  d3.csv("ABI_Einsatzbetriebe_und_Einsatzplaetze_nach_TB_2025.csv").then(raw => {
    ...
    const rowHeight = 36;
    const legendHeight = 40; // ⭐ légende sur une seule ligne par item (embed compact)
    const margin = {
      top: legendHeight,
    ...
    // Plancher d'intensité (0.22 au lieu de 0) : même les catégories les plus
    // faibles (ex. Katastrophen : 11 / 48) restent visiblement teintées au
    // lieu de quasi disparaître dans LIGHT_BETRIEBE/LIGHT_PLAETZE, presque
    // blancs sur fond blanc.
    const MIN_COLOR_INTENSITY = 0.22;
    const colorBetriebe = d3.scaleSqrt().domain([0, maxBetriebe]).range([MIN_COLOR_INTENSITY, 1]);
    const colorPlaetze = d3.scaleSqrt().domain([0, maxPlaetze]).range([MIN_COLOR_INTENSITY, 1]);
